...
class RangeFormItem extends PureComponent {
  render() {
    ...
    getFieldDecorator(keysName, { initialValue: ['initRange'] });
            ...
            >
              {
                getFieldDecorator(`${fieldName}[${index}]`, {
                  rules: index === 0 ? [] : [{ required: true, whitespace: true, message: "Please input range's name or delete this field." }]
                })(<Input placeholder="Please input range." style={{ width: '80%', marginRight: 8 }}/>)
              }
              {keysValue.length > 1 ? (
                <Icon
...
export default connect(stateMapToProps, dispatchMapToProps)(RangeFormItem);
