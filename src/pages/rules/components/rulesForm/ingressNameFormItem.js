...
class IngressNameFormItem extends PureComponent {
  render() {
    const { formItemCommonLayout, formItemLayoutWithOutLabel } = this.props;
    const keysName = 'ingress_nameKeys',
          fieldName = 'ingress_name';
    const { getFieldDecorator, getFieldValue } = this.props.form;
    getFieldDecorator(keysName, { initialValue: ['initIngressName'] });
      ...
      <Fragment>
        {
          keysValue.map((k, index) => (
            <FormItem
              ...
              label={index === 0 ? 'Ingress Name' : ''}
              key={k}
            >
              {
                getFieldDecorator(`${fieldName}[${index}]`, {
                  rules: index === 0 ? [] : [{ required: true, whitespace: true, message: "Please input ingressName's name or delete this field." }]
                })(<Input placeholder="Please input ingress name." style={{ width: '80%', marginRight: 8 }}/>)
              }
              {keysValue.length > 1 ? (
                <Icon
        ...
        >
          <Button type="dashed" onClick={() => this.handleAddItem(keysName)} style={{ width: '80%' }}>
            <Icon type="plus" /> Add ingress name
          </Button>
        </FormItem>
      </Fragment>
    )
...
export default connect(stateMapToProps, dispatchMapToProps)(IngressNameFormItem);
