...
class PatternFormItem extends PureComponent {
  render() {
    const { formItemCommonLayout, formItemLayoutWithOutLabel } = this.props;
    const keysName = 'patternKeys',
      fieldName = 'pattern';
    const { getFieldDecorator, getFieldValue } = this.props.form;
    getFieldDecorator(keysName, { initialValue: ['initPattern'] });
            ...
            >
              {
                getFieldDecorator(`${fieldName}[${index}]`, {
                  rules: index === 0 ? [] : [{ required: true, whitespace: true, message: "Please input pattern's name or delete this field." }]
                })(<Input placeholder="Please input pattern." style={{ width: '80%', marginRight: 8 }}/>)
              }
              {keysValue.length > 1 ? (
                <Icon
...
export default connect(stateMapToProps, dispatchMapToProps)(PatternFormItem);
